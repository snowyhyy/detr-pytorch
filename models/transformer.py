...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        dropout: dropout 率
        activation: 激活函数
        normalize_before: 是否在残差连接之前进行层归一化
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        query_embed = query_embed.unsqueeze(1).repeat(1, bs, 1) # query_embed 作为可学习的位置查询，使解码器关注不同区域的目标。
        mask = mask.flatten(1)

        tgt = torch.zeros_like(query_embed) # 解码器初始输入，初始查询向量全为 0
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed) # 输入特征加入位置编码 pos_embed
